...
from experiments.prompts import (
    ...
    ACE_GENERATOR_HOTPOT_TEMPLATE,
    ACE_GENERATOR_SWITCHLIGHT_TEMPLATE,
    ACE_GENERATOR_CHEMTILE_TEMPLATE,
    ACE_REFLECTOR_HOTPOT_TEMPLATE,
    ACE_REFLECTOR_SWITCHLIGHT_TEMPLATE,
    ACE_REFLECTOR_CHEMTILE_TEMPLATE,
    ACE_CURATOR_HOTPOT_TEMPLATE,
    ACE_CURATOR_SWITCHLIGHT_TEMPLATE,
    ACE_CURATOR_CHEMTILE_TEMPLATE,
    ACE_COUNTERFACTUAL_QUERY_TEMPLATE,
...
class ACEAgent(Agent):
    ...
    def __init__(
        ...
        max_epochs: int = 1,
        use_environment_specific_prompts: bool = True
    ):
        """
        ...
        Args:
            ...
            use_environment_specific_prompts: Use environment-specific templates (True = faithful to paper)
        """
        ...
        self.use_environment_specific_prompts = use_environment_specific_prompts
    ...
    def _get_generator_template(self) -> str:
        """
        Get appropriate Generator template based on environment.

        Returns:
            Template string for Generator prompt
        """
        if not self.use_environment_specific_prompts or not self.environment_name:
            return ACE_GENERATOR_TEMPLATE

        env_map = {
            'HotPotLab': ACE_GENERATOR_HOTPOT_TEMPLATE,
            'SwitchLight': ACE_GENERATOR_SWITCHLIGHT_TEMPLATE,
            'ChemTile': ACE_GENERATOR_CHEMTILE_TEMPLATE,
        }
        return env_map.get(self.environment_name, ACE_GENERATOR_TEMPLATE)

    def _get_reflector_template(self) -> str:
        """
        Get appropriate Reflector template based on environment.

        Returns:
            Template string for Reflector prompt
        """
        if not self.use_environment_specific_prompts or not self.environment_name:
            return ACE_REFLECTOR_TEMPLATE

        env_map = {
            'HotPotLab': ACE_REFLECTOR_HOTPOT_TEMPLATE,
            'SwitchLight': ACE_REFLECTOR_SWITCHLIGHT_TEMPLATE,
            'ChemTile': ACE_REFLECTOR_CHEMTILE_TEMPLATE,
        }
        return env_map.get(self.environment_name, ACE_REFLECTOR_TEMPLATE)

    def _get_curator_template(self) -> str:
        """
        Get appropriate Curator template based on environment.

        Returns:
            Template string for Curator prompt
        """
        if not self.use_environment_specific_prompts or not self.environment_name:
            return ACE_CURATOR_TEMPLATE

        env_map = {
            'HotPotLab': ACE_CURATOR_HOTPOT_TEMPLATE,
            'SwitchLight': ACE_CURATOR_SWITCHLIGHT_TEMPLATE,
            'ChemTile': ACE_CURATOR_CHEMTILE_TEMPLATE,
        }
        return env_map.get(self.environment_name, ACE_CURATOR_TEMPLATE)
    ...
    def answer_query(self, question: str) -> Tuple[str, float]:
        ...
        Uses query-type-specific prompts to improve reasoning:
        - Counterfactual: Emphasizes mental simulation of alternative histories
        - Interventional: Predicts outcomes of proposed actions
        - Planning: Recommends best course of action
        ...
        # For ACE, we need to adapt the templates to include playbook context
        # We'll use the playbook as part of the "beliefs" context
        ...
        if query_type == 'counterfactual':
            # Use ACE-specific counterfactual template with playbook integration
            if self.use_environment_specific_prompts:
                template = ACE_COUNTERFACTUAL_QUERY_TEMPLATE
                # ACE counterfactual template has different parameter names
                prompt = template.format(
                    playbook=playbook_text,
                    observation_history=observation_history,
                    question=question
                )
                # Skip the standard prompt building below
                ...
                # Apply counterfactual confidence calibration
                if confidence > 0.85:
                    confidence = 0.85
                return answer, confidence
            else:
                template = COUNTERFACTUAL_QUERY_TEMPLATE
                belief_context = f"**Playbook:**\n{playbook_text}\n\n**Playbook Size:** {self._get_playbook_size()} items"
            ...
            # Fallback to original ACE template
            template = ACE_QUERY_TEMPLATE
            prompt = template.format(
                playbook=playbook_text,
                memory_summary=observation_history,
                question=question
            )

        # Build prompt for query-type-specific templates
        if query_type in ['counterfactual', 'interventional', 'planning']:
            prompt = template.format(
    ...
    def _choose_action(self, observation: dict) -> Tuple[str, Optional[str]]:
        ...
        # Build Generator prompt using environment-specific template
        template = self._get_generator_template()
        prompt = template.format(
            playbook=playbook_text,
    ...
    def _reflect_on_episode(self, episode: dict, outcome: dict,
        ...
        # Use environment-specific template
        template = self._get_reflector_template()
        prompt = template.format(
            ...
            environment_name=self.environment_name or "Unknown"
        ) + prior_text
    ...
    def _curate_default(self, insights: Dict) -> List[Dict]:
        ...
        Returns:
            List of delta items
        """
        # Build curation prompt using environment-specific template
        template = self._get_curator_template()
        prompt = template.format(
            playbook=self._format_playbook(),
